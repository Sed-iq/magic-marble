/*
    *Player Played Tournaments Class*
...
export default class PlayerTournamentsPlayed {
    constructor() {
        this.urls = ['/player/tournaments/played'];
    ...
    async loadTournaments(userId, tableId) {
        const response = await fetch(`${API_URL}/getPlayerTournaments/?userId=${userId}&status=finished`, {
            method: 'GET',
            headers: {
            ...
            if (tournaments.length > 0) {
                ...
                <tr class="text-gray-700 dark:text-gray-400">
                    ...
                    <td class="px-4 py-3 text-sm">
                        <p class="font-semibold">${tournament.winner}</p>
                    </td>
                    <td class="px-4 py-3 text-sm">
                    ...
                    <td class="px-4 py-3 text-sm">
                        <p class="font-semibold">${new Date(tournament.endDateTime).toLocaleDateString()} ${new Date(tournament.endDateTime).toLocaleTimeString()}</p>
                    </td>
                </tr>
                `;
                });
