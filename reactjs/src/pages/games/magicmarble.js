...
export default function MagicMarble(props) {
    ...
    const [logs, setLogs] = useState([
        { message: 'Welcome to Magic Marble!', type: 'info' },
    ]);
    ...
    function playRound(roundWinner, outcome, wonAmount) {
        setRound(round + 1);
        if (round === 1) {
            setLogs(logs.push({ message: 'Players have been assigned roles randomly', type: 'info' }));
            setLogs(logs.push({ message: `Round ${round}`, type: 'round' }));
            setLogs(logs.push({ message: 'Make your moves', type: 'info' }));
        }
        else if (this.playerOne.score === 0 || this.playerTwo.score === 0) {
            setLogs(logs.push({ message: `${roundWinner} ${outcome} and won ${wonAmount} marbles.`, type: 'info' }));
            setLogs(logs.push({ message: `${playerOne.username} bet ${playerOne.bet} as ${playerOne.role} vs ${playerTwo.username}'s ${playerTwo.bet} as ${playerTwo.role}`, type: 'info' }));
            setLogs(logs.push({ message: 'Game Over', type: 'info' }));
            setLogs(logs.push({ message: `${roundWinner} is the winner`, type: 'info' }));
    ...
    function update(tournament) {
        ...
        if (idx !== -1) {
            ...
            if (matchWithIdx !== -1) {
                playerOne = tournament.currentPlayers[idx];
                playerTwo = tournament.currentPlayers[matchWithIdx];
            }
        }
        else {
    ...
    function handleSubmit(e) {
        ...
        if (playerOne.playerToPlay === playerOne.no) {
            ...
            props.socket.emit('addedbet', { tournamentId, player: playerOne });
        }
    }
                            ...
                            if (data) {
                                if (!data.valid) {
                                    ...
                                    socket.on('playRound', (data) => {
                                        if (window.location.pathname === "/games/magicmarble") {
                                            if (userId) {
                                                playRound(data.roundWinner, data.outcome, data.wonAmount);
                                            }
                                        }
                                    });
        ...
        <div id="main-side">
            ...
            <div id="main-bot">
                <div id="bot-left">
                    ...
                    <div id='layer-bet ' className={(playerOne.playerToPlay === playerOne.no) ? '' : 'hidden'}>
                        ...
                        <div id="stats-left">
                            ...
                            <div id="tournament-stats">
                                <div className="tournament-stats">
                                    <div id="current-round">{round}</div>
